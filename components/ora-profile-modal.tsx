...
interface OraProfileModalProps {
  isOpen: boolean
  onClose: () => void
  ora: Ora
  cmpData: CMPData
...
export function OraProfileModal({
  isOpen,
  onClose,
  ora,
  cmpData,
  ...
  onNameChange,
  onCMPDataChange,
}: OraProfileModalProps) {
  const [localCMPData, setLocalCMPData] = useState<CMPData>(cmpData)
  const [isEditingName, setIsEditingName] = useState(false)
  const [tempName, setTempName] = useState(customName)

  // Update local state when props change
  useEffect(() => {
    setLocalCMPData(cmpData)
        ...
        <DialogHeader className="pb-6 border-b border-white/20">
          <div className="flex items-center gap-6">
            ...
            <div className="w-20 h-20 rounded-3xl overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 shadow-xl flex-shrink-0">
              <img src={ora.image || "/placeholder.svg"} alt={displayName} className="w-full h-full object-cover" />
            </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 font-mono">#{ora.oraNumber}</p>
                    </div>
                  </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                // Save CMP data and communicate changes back to parent
                const updatedCMPData = { ...localCMPData, customName: tempName.trim() }
                onCMPDataChange(updatedCMPData)
                onNameChange(tempName.trim())
                console.log("Saving CMP data:", updatedCMPData)
                onClose()
              }}
            >
              <Save className="w-4 h-4" />
